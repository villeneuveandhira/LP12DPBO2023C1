...
//        GameObject player = null;
        ...
        handler.loop();
        // Take the score increase from the loop() call
        // Imageine the loop() have return value
        score += handler.getScoreIncrease();
//        if(this.running)
//        {   
//            counter++;
//            if(startCounting)
...
//        }
    ...
    {
        double fps = 60.0;
        double ns = (1000000000 / fps); // delay beetween frames (nanoseconds)
        double delta = 0;               // 
...
//        int frames = 0;
        while(running)
        {
            ...
            // execute every one "unit delay"
            while(delta > 1)
            {
                loop();     // execute the game logic
                delta--;
            }
